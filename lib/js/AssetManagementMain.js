function AssetManagementMain(params){
	...
	this.updateHTML = function(data){
		_data = data;
		if(params.switch_to_chat && data["_id"] != undefined){
			/*try{
				params.placement.removeChild(GCM.element);
			} catch (err) {}
			*/
			data.id = data["_id"]["$id"];
			data.element = this.element;
			console.log("Switching to Chat");
			Helper.require('./lib/js/GroupChatMain.js',function(){
				console.log("Enter Group Chat");
				this.GCM = new GroupChatMain(data);
			});
		}else{
			try{
